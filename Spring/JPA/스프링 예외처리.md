...
문제도 고려안했을까? 라고 생각하고 찾아봤더니 전역으로 예외처리를 해주는 롬복이 존재했다. 그리고 이를 이용한 예외처리 전략도
...
Controller, Service, Repository 에서 따로 예외처리를 해주지 않으면 결국 WAS(톰캣) 으로 반환된다.
자바 예외 특성상 처리를 안해주면 호출한쪽으로 다시 던져주기때문이다.
예를 들어 Repository 에서 예외가 발생하면, 
Repository -> Service -> Controller -> Dispatcher Servlet -> Filter -> WAS 순으로 계속 예외를 던져버린다.
최종 도착지는 클라이언트에게 보이는 WAS 인것이다.

## Dispatcher Servlet 예외 처리 방법

예외를 처리하지않으면 클라이언트에게 예외를 던지는건 알았다. 그렇다면 예외처리는 어떻게할까?
...
`@ResponseStatus` 는 롬복으로 사용을 한다. 이럴 경우 외부 라이브러리에는 적용이 불가능하다는 단점이 있다.
...
거쳐야 되기 때문이다. 이렇게되면 Controller 를 한번 더 거치는 작업이 발생하므로 성능상 문제가 될 수 도 있으니 권장하는 방법은 아니다.
...
그래서 Service, Entity 도 `@ExceptionHandler` 로 처리하고 싶다면 세번째 방법을 사용해야한다.
